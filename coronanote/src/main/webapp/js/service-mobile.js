...
var MAX_RESOLUTION = 7000 * 5000; // 16MP (Mega Pixel)
...
$btnShowMenu.on('click', function() {
    ...
    $displayingHiddenMenu.hide();
    $msg.show();
	if($(this).parent().hasClass('txtGroup')){
		$('.btn-prov').show();
	};
    imageEditor.stopDrawingMode();
});
    ...
    if (file) {
        ...
        img.onload = function() {
			...
			if(this.width < 1200 && this.width > 800){
				...
				$inputTextSizeRange.val('150');
			}else if(this.width < 800){
				$cornerSize = 30;
				$rotatingPointOffset = 80;
				$inputTextSizeRange.val('70');
			}
